(function (win, doc) {
  ...
  $wt.extend({
    ...
    ws: "https://jackson.webtools.europa.eu",
    token: "ea207736",
    env: "eks",
    isWpromActive: "",
    ...
    isIOS: function () {
      return (
        /iPad|iPhone|iPod|MacIntel/.test(navigator.platform) && $wt.isMobile
      );
    },
    inProgress: !1,
    components: {},
      ...
      (function recursive(from, to) {
        for (var name in to) {
          if (to.hasOwnProperty(name)) {
            if (
              typeof to[name] === "object" &&
              !Array.isArray(to[name]) &&
    ...
    next: function (elm) {
      ...
      if (elm && elm.className) {
        elm.className = elm.className.replace(/ ?wtWaiting/gi, "");
      }
      try {
        let events = (elm.snippet || elm).params.events;
        if (events && events.onready) {
          let ready = events.onready;
    ...
    setTheme: (defaultTheme) => {
      ...
      if (["ec", "eu"].indexOf(defaultTheme) > -1) {
        ...
        host.indexOf("ec.europa.eu") > -1 ||
        host.indexOf("commission.europa.eu") > -1
      ) {
        theme = "ec";
    ...
    regenerate: function (params = {}, exclude = []) {
      exclude = exclude.concat(["etrans"]);
      [...doc.querySelectorAll("script[type='application/json']")].map(
        (elm) => {
          if (elm.reset && !exclude.includes(elm?.params?.service)) {
            elm.reset();
            if (params && !elm?.params?.utility) {
              delete elm.params;
              elm.innerHTML = JSON.stringify(
                $wt.mergeParams(JSON.parse(elm.innerHTML), params),
              );
            }
          }
        },
    ...
    collect: function () {
      ...
      [...doc.querySelectorAll("script[type='application/json']")].forEach(
        (uec) => {
          let isProcess = uec.getAttribute("data-process");
          if (!isProcess) {
            if (!uec.params) {
            ...
            if (!uec.container) {
              ...
              if (!uec.params.contact && uec.params.service) {
                uec.container.className =
                  "wtWidgets wtWaiting " + uec.params.service;
                if (uec.params.type) {
                  uec.container.className +=
                    " " + uec.params.service + "-" + uec.params.type;
                }
              }
            }
            ...
            let dropFromSearchEngine = excludeFromSearch.filter((name) => {
              return (
                name.indexOf(uec.params.service || uec.params.utility) > -1
              );
            });
    ...
    process: function (elm, params) {
      ...
      let self = this;
      let comp = params.service || params.utility || !1;
      ...
      elm.remove = function () {
        ...
        if (elm.container.message) {
          elm.container.message.remove();
        }
        elm.container = !1;
      };
      ...
      elm.label = function (...args) {
        return this.container.label(...args);
      };
      if (elm.container) {
        elm.container.params = params;
        elm.container.className =
          comp + " wt wt-" + comp + " " + (params["class"] || "");
        if (!elm.container.params.contact && elm.container.params.service) {
          ...
          if (elm.container.params.type) {
            elm.container.className +=
              " " +
              elm.container.params.service +
              "-" +
              elm.container.params.type;
          }
        }
        if (params.css) {
    ...
    runComponent: function (comp, scrpt, params) {
      let translations =
        params.translations ||
        params?.options?.translations ||
        params?.config?.translations;
      let typeOfTrans = typeof translations;
      if (
        translations &&
        (typeOfTrans === "string" || typeOfTrans === "object") &&
        !scrpt.container.translations
      ) {
        return $wt.loadTranslations(
          translations,
          () => {
            $wt.runComponent(comp, scrpt, params);
          },
          comp,
          scrpt.container,
        );
      } else if ($wt[comp] && $wt[comp].run) {
        if (params?.wcloud?.url && scrpt.container) {
          scrpt.container.addEventListener(
            "click",
            () => {
        ...
        $wt._bindUtilsTranslations(scrpt.container);
...
$wt.extend({
  ...
  uniqueID: (scrp) => {
    ...
    if (params) {
      let name =
        params.name || (params.service || params.utility) + "_" + $wt.id();
  ...
  alphaOrder: function (dataSrc, options) {
    ...
    var orderConf = {
        default:
          "aAªáÁàÀăĂâÂåÅǻǺäÄǟǞãÃǡǠąĄāĀæÆǽǼǣǢbBḃḂcCćĆĉĈčČċĊçÇ℅dDďĎḋḊđĐðÐeEéÉèÈĕĔêÊěĚëËęĘēĒėĖəƏfFḟḞƒﬁﬂgGğĞĝĜǧǦġĠģĢǥǤhHĥĤȟȞħĦiIíÍìÌĭĬîÎïÏĩĨİįĮīĪıĳĲjJĵĴkKǩǨķĶĸlLĺĹľĽļĻłŁŀĿmMṁṀnⁿNńŃňŇñÑņŅŋŊŉ№oOºóÓòÒŏŎôÔöÖőŐõÕǫǪǭǬōŌøØǿǾœŒpPṗṖqQrRŕŔřŘŗŖɼsSśŚŝŜšŠṡṠşŞșȘſẛßtTťŤṫṪţŢțȚŧŦ™uUúÚùÙŭŬûÛůŮüÜűŰũŨųŲūŪvVwWẃẂẁẀŵŴẅẄxXyYýÝỳỲŷŶÿŸzZźŹžŽżŻʒƷǯǮ",
        greek:
          "αΑἀἈἄἌἂἊἆἎἁἉἅἍἃἋἇἏάΆὰᾺᾶάΆᾳᾼᾀᾈᾄᾌᾂᾊᾆᾎᾁᾉᾅᾍᾃᾋᾇᾏᾴᾲᾷᾰᾸᾱᾹβϐΒγΓδΔεΕἐἘἔἜἒἚἑἙἕἝἓἛέΈὲῈέΈϝϜϛϚζΖηΗἠἨἤἬἢἪἡἩἥἭἣἫἧἯήΉὴῊῆἦἮήΉῃῌᾐᾘᾔᾜᾒᾚᾖᾞᾑᾙᾕᾝᾓᾛᾗᾟῄῂῇθϑΘιιΙἰἸἴἼἲἺἶἾἱἹἵἽἳἻἷἿίΊὶῚῖίΊῐῘϊΪΐῒῗΐῑῙκϰΚϗλΛμµΜνΝξΞοΟὀὈὄὌὂὊὁὉὅὍὃὋόΌὸῸόΌπϖΠϟϞρϱΡῤῥῬσςΣτΤυΥὐὔὒὖὑὙὕὝὓὛὗὟύΎὺῪῦύΎῠῨϋΫΰῢῧΰῡῩφϕΦχΧψΨωΩΩὠὨὤὬὢὪὦὮὡὩὥὭὣὫὧὯώΏὼῺῶώΏῳῼᾠᾨᾤᾬᾢᾪᾦᾮᾡᾩᾥᾭᾣᾫᾧᾯῴῲῷϡϠ",
        cyrillic:
          "аАӑӐӓӒәӘӛӚӕӔбБвВгГґҐғҒҕҔдДђЂѓЃҙҘеЕѐЀёЁӗӖєЄжЖӂӁӝӜҗҖзЗӟӞѕЅӡӠиИѝЍӣӢӥӤіІїЇйЙјЈкКқҚӄӃҡҠҟҞҝҜлЛљЉмМнНңҢӈӇҥҤњЊоОӧӦөӨӫӪпПҧҦрРсСҫҪтТҭҬћЋќЌуУӯӮўЎӱӰӳӲүҮұҰфФхХҳҲһҺцЦҵҴчЧӵӴҷҶӌӋҹҸҽҼҿҾџЏшШщЩъЪыЫӹӸьЬэЭюЮяЯҩҨӀ",
      },
      options = options || {
  ...
  cleanHTML: function (str, params = {}) {
    if (typeof str !== "string") {
      return "";
    }
    let config = $wt.mergeParams(
      {
        xss: !0,
        valid_elements: "",
      },
    ...
    str = this.filterHtml(str, config.valid_elements);
    return config.xss ? this.filterXss(str) : str;
  },
  parse: {
    csv: function (data, cfg) {
      if (data.split(".").pop() === "csv") {
        console.log(
          "WTERROR: In order to parse the csv you will need to provide the content of the file, not the url.",
        );
        return !1;
      }
      var lineDelimiter = /\r\n|\n/;
      ...
      for (var i = 0; i < tab.length; i++) {
        ...
        for (var j = 0; j < subtab.length; j++) {
          var col = subtab[j];
          if (
            ["auto", ","].indexOf(cfg.decimal.point) !== -1 ||
            isNaN(parseFloat(subtab[j]))
          ) {
            col = col.replace(/,/g, ".");
          }
          col = col.replace(/"/g, "");
  ...
  serialize: (form) => {
    return Array.from(new FormData(form)).reduce((fields, [key, val]) => {
      return Object.assign({}, fields, {
        [key]: fields[key]
          ? (Array.isArray(fields[key]) ? fields[key] : [fields[key]]).concat(
              val,
            )
          : val,
      });
...
$wt.extend({
  ...
  noconflict: (container) => {
    if (container?.querySelectorAll) {
      [...container.querySelectorAll('*:not([class*="wt"])')].map((elm) =>
        elm.classList.add("wt-noconflict"),
      );
    }
  },
});
$wt.extend({
  ...
  on: function (o, e, f) {
    e === "load" && document.readyState === "complete"
      ? f()
      : o.addEventListener
...
$wt.extend({
  dictionary: {
    sml: {
      ar: {
        machine_translated: "تمت ترجمة العنصر أعلاه آليًا.",
        more_information: "مزيد من المعلومات",
      },
      be: {
        machine_translated:
          "Прыведзены вышэй элемент быў машынна перакладзены.",
        more_information: "Больш інфармацыі",
      },
      bg: {
        machine_translated: "Горният елемент е преведен машинно.",
        more_information: "За повече информация",
      },
      ca: {
        machine_translated:
          "L'element anterior ha estat traduït automàticament.",
        more_information: "Més informació",
      },
      cs: {
        machine_translated: "Text výše byl přeložen strojově.",
        more_information: "Další informace",
      },
      da: {
        machine_translated: "Ovenstående visualisering er maskinoversat.",
        more_information: "Yderligere oplysninger",
      },
      de: {
        machine_translated: "Das obige Element wurde maschinell übersetzt.",
        more_information: "Weitere Informationen",
      },
      el: {
        machine_translated:
          "Το ανωτέρω σημείο είναι προϊόν αυτόματης μετάφρασης.",
        more_information: "Περισσότερες πληροφορίες",
      },
      en: {
        machine_translated: "The item above was machine-translated.",
        more_information: "More information",
      },
      es: {
        machine_translated:
          "Para traducir esta figura se ha utilizado un sistema de traducción automática.",
        more_information: "Más información",
      },
      et: {
        machine_translated: "Tegemist on masintõlkega.",
        more_information: "Lisateave",
      },
      fi: {
        machine_translated:
          "Edellä olevan visuaalisen elementin teksti on konekäännös.",
        more_information: "Lisätietoja",
      },
      fr: {
        machine_translated:
          "L’élément ci-dessus a été traduit par traduction automatique.",
        more_information: "En savoir plus",
      },
      ga: {
        machine_translated: "Rinneadh meaisínaistriúchán ar an mír sin thuas.",
        more_information: "Tuilleadh eolais",
      },
      he: {
        machine_translated: "הפריט שלמעלה תורגם אוטומטית.",
        more_information: "מידע נוסף",
      },
      hi: {
        machine_translated:
          "ऊपर दी गई सामग्री का मशीन द्वारा अनुवाद किया गया है।",
        more_information: "अधिक जानकारी",
      },
      hr: {
        machine_translated: "Tekst ovog vizualnog prikaza strojno je preveden.",
        more_information: "Više informacija",
      },
      hu: {
        machine_translated: "A fenti tartalom gépi fordítással készült.",
        more_information: "További információk",
      },
      hy: {
        machine_translated: "Վերևի տարրը թարգմանվել է մեքենայով։",
        more_information: "Ավելին տեղեկություն",
      },
      is: {
        machine_translated: "Atriðið hér að ofan var vélþýtt.",
        more_information: "Frekari upplýsingar",
      },
      it: {
        machine_translated:
          "Il contenuto sopra è stato tradotto automaticamente.",
        more_information: "Maggiori informazioni",
      },
      ja: {
        machine_translated: "上記の項目は機械翻訳されました。",
        more_information: "詳細情報",
      },
      lt: {
        machine_translated:
          "Pirmiau pateiktas tekstas automatiškai išverstas mašininio vertimo priemone.",
        more_information: "Daugiau informacijos",
      },
      lv: {
        machine_translated:
          "Augstāk redzamais elements pārtulkots, izmantojot mašīntulkošanu.",
        more_information: "Papildu informācija",
      },
      mk: {
        machine_translated: "Горниот елемент е машински преведен.",
        more_information: "Повеќе информации",
      },
      mt: {
        machine_translated: "Il-punt t’hawn fuq ġie tradott b’mod awtomatiku.",
        more_information: "Aktar informazzjoni",
      },
      nl: {
        machine_translated: "Het bovenstaande is een machinevertaling.",
        more_information: "Meer informatie",
      },
      no: {
        machine_translated: "Elementet ovenfor ble maskinoversatt.",
        more_information: "Mer informasjon",
      },
      pl: {
        machine_translated:
          "Ten element graficzny został przetłumaczony maszynowo.",
        more_information: "Dodatkowe informacje",
      },
      pt: {
        machine_translated: "O conteúdo acima foi traduzido automaticamente.",
        more_information: "Mais informações",
      },
      ro: {
        machine_translated: "Elementul de mai sus a fost tradus automat.",
        more_information: "Informații suplimentare",
      },
      ru: {
        machine_translated:
          "Приведённый выше элемент был переведен машинным способом.",
        more_information: "Больше информации",
      },
      sk: {
        machine_translated: "Vyššie uvedená položka bola strojovo preložená.",
        more_information: "Ďalšie informácie",
      },
      sl: {
        machine_translated: "Gornja vsebina je bila strojno prevedena.",
        more_information: "Več informacij",
      },
      sq: {
        machine_translated: "Artikulli më sipër u përkthye me makinë.",
        more_information: "Më shumë informacion",
      },
      sv: {
        machine_translated: "Ovanstående är maskinöversatt.",
        more_information: "Mer information",
      },
      tr: {
        machine_translated: "Yukarıdaki öğe makine tarafından çevrilmiştir.",
        more_information: "Daha fazla bilgi",
      },
      uk: {
        machine_translated: "Наведений вище елемент було машинно перекладено.",
        more_information: "Докладніше",
      },
      ur: {
        machine_translated: "اوپر دیا گیا مواد مشین سے ترجمہ کیا گیا ہے۔",
        more_information: "مزید معلومات",
      },
      vi: {
        machine_translated: "Mục trên đã được dịch tự động.",
        more_information: "Thêm thông tin",
      },
      zh: {
        machine_translated: "以上项目为机器翻译。",
        more_information: "更多信息",
      },
      lu: {
        machine_translated: "Déi uewen uginn Element gouf maschinell iwwersat.",
        more_information: "Méi Informatiounen",
      },
    },
  },
  isRtl: function (direction, lang) {
    ...
    return (
      direction === "rtl" ||
      /(ar|arc|dv|fa|ha|he|khw|ks|ku|ps|ur|yi)/i.test(lang)
    );
  },
  label: function (
    set,
        ...
        if (strg) {
          ...
          if (typeof strg === "object") {
            ...
            translate = translate.replace(/{(\d+)}/g, function (match, key) {
              return typeof placeholder[key - 1] !== "undefined"
                ? placeholder[key - 1]
                : "";
            });
          }
    ...
    let defaultValue = initialValue ? label : !1;
    return translate || defaultValue || "UNKNOWN LABEL";
  },
  addTranslation: (json, force = !1) => {
    ...
    for (let i in json) {
      ...
      if (force) {
        dico[i] = $wt.mergeParams(json[i], dico[i]);
      } else {
  ...
  loadTranslations: (translations, callback, comp, container) => {
    ...
    window.addEventListener(
      "wtNext",
      (what) => {
        let elm = what.parameters?.params?.domRef || what.parameters;
        if (elm && elm.message) {
          _message(elm);
        }
      },
    ...
    const _message = (elm) => {
      let lang = elm?.params?.lang || document.lang;
      let localDico = elm.translations || {};
      let isAutomatic = elm?.automatic || [];
      let forceMessage = isAutomatic.includes(lang);
      let message = elm.message;
      let localAuto = localDico[lang]?.__meta__?.source === "automatic";
      if (message && (forceMessage || localAuto) && !$wt.isMachine) {
        elm.classList.add("wt-message--warning-translation-container");
        let machine_translated = $wt.label("sml", "machine_translated", lang);
        let more_information = $wt.label("sml", "more_information", lang);
        let more_info_url =
          "https://commission.europa.eu/languages-our-websites/use-machine-translation-europa_";
        message.innerHTML = `${machine_translated} <a href="${more_info_url}${lang}">${more_information}</a>`;
      }
    };
    const _translateStrategy = (elm) => {
      let haveMachine = Object.entries(elm.translations).filter(
        ([lang, val]) => val?.__meta__?.source === "automatic",
      );
      let setMachine = Array.isArray(elm.params?.translations?.automatic);
      if (container && (haveMachine.length || setMachine)) {
        container.message = document.createElement("div");
        container.message.classList.add("wt-message--warning-translation");
        $wt.after(container.message, container);
      }
    };
    const _add = (response) => {
      let data = {};
      let meta = response.__meta__;
      if (comp && !response[comp]) {
        data[comp] = response;
        response = data;
        if (meta) {
          response.__meta__ = meta;
        }
      }
      if (container) {
        container.translations = response[comp] || response;
        _translateStrategy(container);
        if (comp === "map") {
        ...
        $wt.getFile({
          ...
          success: (json) => {
            ...
            try {
              ...
              $wt.isLoad[url] = labels;
              _add(labels);
            } catch (e) {
              console.log(
                "WTERROR: Couldn't load custom dictionary'",
                url,
                "'",
              );
              error = !0;
          ...
          error: () => {
            console.log("WTERROR: Couldn't load custom dictionary'", url, "'");
            callback(!1);
          },
        });
      } else {
    ...
    if (Array.isArray(translations?.automatic)) {
      container.automatic = translations.automatic;
    }
    if (typeof translations === "string" || translations?.src) {
      _load(translations?.src || translations);
    } else if (typeof translations === "object" && !translations?.src) {
      _add(translations);
      callback(!0);
    } else {
      callback(!0);
  ...
  _bindUtilsTranslations: (container) => {
    container.label = function (label, options = {}) {
      let params = this.params;
      let message = this.message;
      let local = this.translations || {};
      let service = options.name || params.service || params.utility;
      let name = $wt.alias[service] || service;
      let main = JSON.parse(JSON.stringify($wt.dictionary[name] || {}));
      let language =
        options?.lang || options?.language || params.lang || document.lang;
      let global = $wt.mergeParams(main, local);
      let translation = local[language] || local.en || global[language] || {};
      let fallback = options?.fallback || (local.en ? local.en[label] : !1);
      let final = translation[label] || fallback || "UNKNOWN LABEL";
      if (options?.replacePlaceholders) {
        let containsPlaceholder = function (str) {
          const open = str.indexOf("{");
          const close = str.indexOf("}");
          return open !== -1 && close !== -1 && open < close;
        };
        if (containsPlaceholder(label)) {
          final = $wt.template(final, translation, {
            preserve: !0,
          });
          final = $wt.template(final, translation, {
            preserve: !0,
      ...
      if (
        message &&
        translation?.__meta__?.source === "automatic" &&
        !$wt.isMachine
      ) {
        let machine_translated = $wt.label("sml", "machine_translated");
        let more_information = $wt.label("sml", "more_information");
        let more_info_url =
          "https://commission.europa.eu/languages-our-websites/use-machine-translation-europa_";
        let more_info_lang = params.lang || document.lang;
        message.innerHTML = `${machine_translated} <a href="${more_info_url}${more_info_lang}">${more_information}</a>`;
      }
      return options.unknown === !1 && final === "UNKNOWN LABEL" ? "" : final;
    };
  },
});
window.addEventListener(
  "wtReady",
  () => ($wt.isMachine = !!document.querySelector('meta[content="machine"]')),
);
$wt.extend({
  isLoad: [],
  handleCrossorigin: function (url, tag) {
    ...
    if (
      ...
      source.hostname.indexOf("europa.eu") > -1 &&
      url.indexOf("webtools/webtools.") > -1
    ) {
  ...
  jsonstat: function (params) {
    ...
    if (!window.JSONstat && !window.waitJSONSTAT) {
      ...
      $wt.load(
        [
          $wt.root + "/libs/json-stat/json-stat.js",
          $wt.root + "/libs/json-stat/utils/json-stat.utils.js",
        ],
        () => bePatient(params),
      );
      return;
    ...
    typeof from === "object"
      ? convert(from)
      : $wt.jsonp(from, (json, error) => {
          !error
            ? convert(json)
            : console.log(
                "WTERROR: jsonstat - request failed on url: \n",
                from,
              );
        });
  },
    ...
    if (cat) {
      if (from === "lasko") {
        ...
        try {
          ...
          console.log(
            "WTERROR: undefined categories " + cat + " in your dataset",
          );
        }
      }
    ...
    if (series) {
      ...
      output.series = dataSeries.map((row, index) => {
        return {
          label: row.label,
          id: dataSeriesDim.id
            ? dataSeriesDim.id[index]
            : String(row.label).replace(/[^a-z0-9 ]/gi, ""),
        };
      });
      if (sheets) {
    ...
    if (url !== "" && typeof url === "string") {
        ...
        if (request.readyState === 4) {
          if (
            ...
            if (typeof success === "function") {
              binary
                ? success(request)
                : success(request.responseText, request.responseXML, config);
            } else {
              return {
      ...
      if ($wt.cacheable(url)) {
        request.setRequestHeader(
          "Cache-Control",
          `public, must-revalidate, max-age=${ttl}`,
        );
      } else {
        request.setRequestHeader(
          "Cache-Control",
  ...
  cacheable: function (url) {
    let nonCacheablePatterns = [
      "https://www.acceptance.europa.eu/assets/wcloud/widgets",
      ".fpfis.tech.ec.europa.eu",
    ]
      .concat(
        "/wcloud/preview/,www.acceptance.europa.eu/assets/wcloud/widgets".split(
          ",",
        ),
      )
      .map((str) => encodeURIComponent(str));
    return !nonCacheablePatterns.some((pattern) =>
      encodeURIComponent(url).includes(pattern),
    );
  },
  getFile: async (params = {}) => {
    ...
    const ajaxError = (response, xhr) => {
      try {
        ...
        params.error(
          "WTINFO: " + (response?.message || response.wtstatus?.status),
        );
      } catch (e) {
    ...
    const xlsBufferToJson = (content) => {
      ...
      let asTables = params.options.to === "tables";
      ...
      if (asGeojson) {
        if (!geoParams) {
          console.log(
            "WTINFO: 'coordinates' or 'geocode' must be present when geojson is requested",
          );
          return geojson;
        }
        geoSheet =
          Number(
            (geoParams.lat || geoParams).split(";")[0].split("sheet:")[1],
          ) - 1;
        geoReference = {
      ...
      workbook.SheetNames.map((sheetName, index) => {
        ...
        if (asTables) {
          let json = XLSX.utils.sheet_to_json(sheetRef);
          let dataSet = json.map((row) => Object.values(row));
          tables.push({
            title: sheetName,
            data: dataSet,
            dataRaw: json,
              ...
              if (asGeojson) {
                ...
                if (geoReference.lat) {
                  feature.geometry.coordinates = [
                    row[geoReference.lat],
                    row[geoReference.lon],
                  ];
                } else if (geoReference.geo) {
                  feature.geometry.search = geoReference.geo
                    .split(",")
                    .map((kk) => {
                      return row[kk] || "";
                    })
      ...
      return asGeojson ? geojson : asTables ? tables : pivot;
    };
    const batchGeocodingService = (geojson, key) => {
      if (geojson.features.length > 5000) {
        return params.error(
          "The geocoding batch service is limited to 5000 requests.",
        );
      }
      ...
      (queue = async () => {
        ...
        if (cache[term]) {
          ...
          result = await fetch(
            "https://gisco-services.ec.europa.eu/nominatim/search.php?limit=1&format=json&q=" +
              term,
          ).then((res) => res.json());
          if (!result[0]) {
            cache[term] = [0, 0];
            console.warn(
              "WTINFO: The geocoding service returns an empty response based on your search criteria:",
              feature.geometry.search,
            );
          } else {
            geojson.features[index].geometry.coordinates = cache[term] = [
              Number(result[0].lon),
    ...
    if (/(xlsx?|ods)/i.test(params.type)) {
      ...
      if (!$wt.cacheable(url)) {
        url += params.url.indexOf("?") > -1 ? "&" : "?";
        url += "v=" + (Math.random() + "").slice(2, 8);
      }
      if (
        !params.options.proxy &&
        (isGeojson ||
          params.options.to === "json" ||
          params.options.to === "tables")
      ) {
        let info = {};
        ...
        request.push("formatting=" + params?.options?.formatting);
        request.push("theming=" + params?.options?.theming);
        if (params.options.coordinates) {
          request.push(
            "coordinates=" +
              encodeURIComponent(JSON.stringify(params.options.coordinates)),
          );
    ...
    } else if (/(docx?|odt)/i.test(params.type)) {
      let url = encodeURIComponent(params.url);
      if (!$wt.cacheable(url)) {
        url += params.url.indexOf("?") > -1 ? "&" : "?";
        url += "v=" + (Math.random() + "").slice(2, 8);
      ...
      request.push("theming=" + params.options.theming);
      $wt.ajax({
        url: $wt.root + `/rest/office/?` + request.join("&"),
        error: ajaxError,
        success: (json) => {
          try {
            json = typeof json === "string" ? JSON.parse(json) : json;
      ...
      $wt.ajax({
        url:
          $wt.root +
          "/rest/requests/files?type=csv&urlToRequest=" +
      ...
      $wt.ajax({
        url:
          $wt.root +
          "/rest/requests/files?type=xml&urlToRequest=" +
          encodeURIComponent(params.url) +
          "&ref=" +
          referer,
        error: ajaxError,
      ...
      $wt.ajax({
        url:
          $wt.root +
          "/rest/requests/files?type=json&urlToRequest=" +
          encodeURIComponent(params.url) +
          "&ref=" +
          referer,
        error: ajaxError,
      ...
      process = (jsondata) => {
        ...
        $wt.jsonstat(
          $wt.mergeParams($wt.mergeParams(params, params.options), {
            from: from,
      ...
      return params.error(
        "WTINFO: Unknown type for getting the file: \n" + params.url,
      );
    }
  },
  file: {
...
$wt.extend({
  ...
  cookie: {
    set: function (params) {
      ...
      if (params.days) {
        params.date.setTime(
          params.date.getTime() + params.days * 24 * 60 * 60 * 1000,
        );
    ...
    consent: {
      ...
      is: {
        allAccepted: function () {
          return window.euCookieConsent
            ? this.choiceMade() && !$wt.cookie.exists("eu_optout")
            : this.choiceMade() && $wt.cookie.consent.get(2).all1st === !0;
        },
        technicalAccepted: function () {
          return window.euCookieConsent
            ? !$wt.cookie.exists("eu_optout")
            : $wt.cookie.consent.get(2).cm === !0;
        },
        choiceMade: function () {
          return window.euCookieConsent
            ? $wt.cookie.exists("eu_cookie_consent")
            : $wt.cookie.consent.get(2).cm === !0;
        },
      },
      set: function (json) {
        if (json.hasOwnProperty("cm")) {
          ...
          var host =
            window.location.host.indexOf("europa.eu") >= 0 ? ".europa.eu" : !1;
          document.cookie =
            "eu_cookie_consent=" +
            escape(json) +
            "; path=/" +
...
((frame, timer) => {
  const init = async () => {
    if ($wt.isFrame) {
      if (window.name.includes("WT_FRAME_")) {
        console.error(
          "Webtools only support the standard way using the frame feature.",
        );
      } else if (window.name.includes("WTFRAME_") && window.infographicData) {
        console.warn(
          "WTINFO: Please use the correct 'infogram' component. See https://webtools.europa.eu/documentation.",
        );
      } else if (
        window.name.includes("WTFRAME_MEDIA") ||
        window.name.includes("_RATIO_")
      ) {
        await networkIdle({
          delay: 500,
        });
        return hideLoading();
      } else if (window.name.includes("WTFRAME_IDLE")) {
        await networkIdle();
        hideLoading();
        resizeEvents({
          domchange: "delay",
      ...
      } else if (window.name.includes("WTFRAME_")) {
        resizeEvents({
          resize: !0,
        });
      }
  ...
  const networkIdle = (options = {}) => {
    ...
    return new Promise((resolve, reject) => {
      ...
      let networks = new PerformanceObserver(() => {
        clearTimeout(timer);
        solve();
      });
      fallbackTimer = setTimeout(reject, 15000);
      solve();
      networks.observe({
        entryTypes: ["resource"],
      });
  ...
  const resizeEvents = (options = {}) => {
    fixMargin();
    $wt.on(window, "load", () => {
      resizeMe(options);
      if (options.resize) {
        setTimeout(
          () => window.addEventListener("resize", () => resizeMe(options)),
          250,
        );
      }
      window.addEventListener("orientationchange", () => resizeMe(options));
      domChangeResize(options);
    });
  };
  const domChangeResize = (options = {}) => {
    const debounce = () => {
      if (options.domchange === "delay") {
        clearTimeout(timer);
        timer = setTimeout(() => resizeMe(), 900);
      } else {
        resizeMe();
      }
    };
  ...
  const fixMargin = () => {
    ...
    div.className = "wt-frame--fix-margin";
    document.body.insertBefore(div, document.body.firstChild);
  };
  const fixFirefoxAnchor = () => {
  ...
  const trackClick = () => {
    $wt.on(window, "click", (evt) => {
      ...
      if (
        targ &&
        targ.href &&
        !targ.getAttribute("aria-controls") &&
        !targ.isTracked &&
        (!targ.getAttribute("href").match(/^(javascript|mailto|#|sms)/i) ||
          ($wt.urlParams.render === "iframe" &&
            targ.href.indexOf("europa.eu/search") < 0))
      ) {
        evt.preventDefault();
        evt.stopPropagation();
        parent.postMessage(
          {
            service: "wt:frame:track:click",
            link: targ.href,
          },
          "*",
        );
  ...
  window.addEventListener("message", (evt) => {
    if (evt?.data?.service === "wt:frame:cck:accepted") {
  ...
  window.addEventListener("message", (evt) => {
    ...
    if (data.service === "request-parent-info" && iframe) {
      if (iframe.contentWindow) {
        iframe.contentWindow.postMessage({
  ...
  const message = (e) => {
    let data = e.data || {};
    let iframe = document.querySelector('iframe[name="' + data.name + '"]');
    if (data.service === "wt:frame:track:search") {
    ...
    } else if (data.service === "wt:frame:track:click") {
      $wt.trackLinks(data.link);
    } else if (data.service === "wt:frame:reset") {
      resizeMe();
      ...
      if ($wt.laco) {
        ...
        $wt.load(
          [
            $wt.root + (isDyn ? "" : "/css") + "/webtools.laco.css?t=ea207736",
            $wt.root + (isDyn ? "" : "/js") + "/webtools.laco.js?t=ea207736",
          ],
          () => {
            $wt.laco.openModal(data);
          },
      ...
      if (!$wt.etrans) {
        ...
        $wt
          .waitUntil(() => $wt.etrans)
          .then(() => $wt.etrans.translate("body", data.to));
        ...
        $wt.etrans.resetState();
      }
    }
    pong(e, data);
    $wt.trigger(window, "wt:frame:resize", {
      data,
      iframe,
      event: e,
    });
  };
  const ping = (callback, options = {}) => {
    if ($wt.isFrame) {
      parent.postMessage(
        {
          ...{
            service: "wt:frame:ping",
            name: window.name,
      ...
      window.addEventListener("message", (evt) => {
        if (evt.data.service === "wt:frame:pong") {
          if (typeof callback === "function") {
            callback(evt.data);
          }
        }
  ...
  const pong = (evt, data) => {
    if (!$wt.isFrame && data.service === "wt:frame:ping") {
      document.querySelectorAll("iframe").forEach((frame) => {
        if (frame.contentWindow === evt.source) {
          frame.contentWindow.postMessage(
            {
              service: "wt:frame:pong",
              name: window.name,
              title: document.title,
  ...
  const resizeMe = (options = {}) => {
    if (!$wt.isFrame && !parent.postMessage) {
      return;
    }
    $wt.include(
      $wt.getStaticPath({
        fallback: $wt.root + "/webtools.frame.css",
        name: "frame",
        params: {},
        type: "css",
      }),
      () => {
        ...
        let isExclude =
          window.angular || window.THEOplayer || window.wtFrameAutoDisable;
        html.classList.add("wt-frame--resizing");
        if (
          isExclude ||
          document.body.classList.contains("wt-frame--auto-disable")
        ) {
        } else {
          html.classList.add("wt-frame--height-auto-important");
        }
        clearTimeout(html.timer);
        html.timer = setTimeout(() => {
          parent.postMessage(
            {
              service: "wt:frame:resize",
              name: window.name,
  ...
  $wt.on(window, "wtReady", init);
  $wt.on(window, "message", message);
})(2025);
$wt.extend({
  dialog: (config = {}) => {
    let params = {
  ...
  pop: function (c = {}) {
    ...
    $wt.pop.wtPopup.className =
      "wtPopup " + (c.fullscreen ? "wtPopupFullscreen " : "") + c["class"];
    ...
    $wt.pop.wtPopup.dialogDesc.innerHTML =
      "Escape will cancel and close the window";
    var hh = c.head || c.title;
    hh = hh
      ? "<h1 class='wt-head'><span>" + hh + "</span></h1>"
      : "<h1 class='wt-head' aria-hidden='true'>&nbsp;</h1>";
...
$wt.extend({
  languages: {
    ...
    regexpURL:
      /(https?:)?\/\/[^\/]+\/?(.*)(_|-|::|=|\/)([a-z]{2})(\.|&|#|$|\?|\/)(.*)/gi,
    list: function () {
      return Object.keys(this["official"]).concat(
        Object.keys(this["non-official"]),
      );
    },
    fromURL: function (url) {
      ...
      let lang = webtools
        ? webtools
        : (url + "").replace(this.regexpURL, "$4").toLowerCase();
      let list = this.list();
      return lang.length === 2 && list.includes(lang)
        ? lang.toLowerCase()
        : null;
    },
    survey: () => {
      ...
      $wt.domchange((mutations) => {
        let htmlChange = mutations.filter(
          (mut) =>
            mut.attributeName === "lang" &&
            mut.target.tagName.toLowerCase() === "html",
        );
  ...
  lang: function (def) {
    ...
    let browserLang = window?.navigator?.language
      ...
      : !1;
    let browserLangSupport =
      browserLang && list.includes(browserLang) ? browserLang : !1;
    ...
    if (etrans && !etransnolive) {
      let checkLang =
        /^(\w{2})$/.test(etrans) && list.includes(etrans) ? etrans : lang;
...
$wt.extend({
  shadeColor: function (color, percent) {
    ...
    return (
      ...
      (
        0x1000000 +
        (Math.round((bin - red) * pourcent) + red) * 0x10000 +
        (Math.round((bin - green) * pourcent) + green) * 0x100 +
        (Math.round((bin - blue) * pourcent) + blue)
      )
        .toString(16)
        .slice(1)
    );
    ...
    $wt.domchange(() => {
      $wt.defer(() => {
        ...
        if (UECNew.length) {
          ...
          UECExist = UEC.slice()
            .filter((elm) => elm.getAttribute("data-process"))
            .map((elm) => elm.reference);
          Object.keys($wt.components)
            .slice()
            .forEach((x) => {
...
window.addEventListener("wtReady", () => {
  const isDynatrace = window.dtrum;
  if (!isDynatrace || window.location.protocol !== "https:") {
    return;
  }
  if ($wt.isDNTOn()) {
    return isDynatrace.disable();
  }
  if ($wt.exists("cck")) {
    window.addEventListener("wtCckReady", () => {
      if ($wt.cookie.consent.is.allAccepted()) {
        isDynatrace.enable();
      } else if ($wt.cookie.consent.is.technicalAccepted()) {
        isDynatrace.disable();
      } else {
        window.addEventListener("cck_all_accepted", () => isDynatrace.enable());
...
$wt.print = (container, options = {}) => {
  let isURL = typeof container === "string";
  let params = $wt.mergeParams(
    {
      url: container,
      filename: document.title,
      filetype: options.output_format || options.filetype || "jpg",
      download: !1,
      format: "A4",
      ...
      filter: (node) => {
        if (node.tagName) {
          return node.tagName.toLowerCase() !== "use";
        }
        return !0;
      },
      quality: 0.95,
  ...
  return new Promise((resolve, reject) => {
    const response = (dataUrl) => {
      if (params.filetype === "pdf" && !isURL) {
        $wt.include($wt.root + "/libs/jspdf/jspdf.umd.min.js", () => {
          ...
          doc.save(params.filename + "." + params.filetype);
        });
      } else if (params.download) {
        let link = document.createElement("a");
        link.download = params.filename + "." + params.filetype;
        link.href = dataUrl;
        link.target = params.target || "_self";
        link.click();
      } else {
        if (!window.wtPrint) {
          ...
          window.wtPrint = newIframe;
          window.wtPrint.setAttribute(
            "style",
            "background-color:#fff;position:absolute;left:-900px;top:-900px;width:0;height:0;",
          );
        }
        window.wtPrint.focus();
        let doc = window.wtPrint.contentWindow.document;
        doc.open();
        doc.write(
          '<img style="max-width: 100%;" class="wt-img" onload="document.execCommand(\'print\', false, null)||window.print();" src="' +
            dataUrl +
            '" border="0">',
        );
        doc.close();
    ...
    if (isURL) {
      params.url = $wt.url.params.add("wtrender", "true", params.url);
      params.url = $wt.url.params.add("wteagerimages", "true", params.url);
      params.download = !0;
      params.filetype = options.filetype || "pdf";
      $wt.ajax({
        url: "https://jackson.webtools.europa.eu/rest/html2m/convert",
        data: params,
        error: console.error,
        success: (json) => {
          try {
            ...
            if (json.output) {
              ...
              console.log("WTINFO: Print service failed:", json);
              reject("WTINFO: Print service failed.");
            }
          } catch (e) {
            alert("WTINFO: Print service is not available.");
            reject("WTINFO: Print service is not available.");
          }
        },
      });
      return;
    ...
    $wt.include(
      $wt.root + "/libs/dom-to-image-more/dom-to-image-more.min.js",
      () => {
        if (params.filetype !== "pdf" && params.scale === 1) {
          params.scale = 2;
        }
        let engine =
          params.filetype === "png" ? domtoimage.toPng : domtoimage.toJpeg;
        engine(container, params)
          .then(response)
          .catch((e, m) => {
            console.log(e, m, params);
            reject("WTINFO: htmlToImage failed");
          });
      },
    );
  });
...
$wt.on(window, "wtBeforeCollect", () => {
  if (document.querySelector("script[src*='europa.eu/webtools/load.js']")) {
    console.warn(
      [
        "%c⚠️ Warning ⚠️%c",
        "%cDecommissioned Webtools Smart Loader used",
        "%cThe URL of the Webtools Smart Loader script (SML) changed from https://europa.eu/webtools/load.js to https://webtools.europa.eu/load.js",
        "The redirection from the old to the new URL has been disabled on %cMonday 2 February 2026",
        "%cPlease make sure to update the URL on your websites, as %cWebtools components no longer work on websites still using the old URL since that date",
      ].join("\n"),
      "background: #ceb73f33; font-size:3em; padding:0.15em; margin: 0em auto; border: 2px solid #ceb73f; border-radius: 4px; font-weight: bold; text-shadow: 1px 1px 1px #000000bf;",
      "",
...
$wt.extend({
    ...
    if (typeof str === "string") {
      ...
      [].forEach.call(document.body.querySelectorAll("a[href]"), function (a) {
        if (
          ...
          a.href &&
          a.href.match &&
          !a.href.match(/^(javascript|mailto|#|sms)/i) &&
          !a.getAttribute("aria-controls") &&
          !/ea_ignore|piwik_ignore/.test(a.className)
        ) {
          t.addListener(a);
  ...
  trackPageView: function (params, can) {
    ...
    if (!params.siteID) {
      ...
      console.log(
        "WTERROR: The Europa Analytics instance, [" +
          inst +
      ...
      var backwardCompatibility = params.rsiteID
  ...
  analytics: {
    ...
    init: function () {
      ...
      Element.prototype.addEventListener = function (
        ...
        if (
          /ea_ignore|piwik_ignore/.test(this.className) &&
          listener.toString().indexOf("function(cZ){cZ=cZ||R.event;") > -1
        ) {
          return;
        }
        if (useCapture) {
            ...
            for (var y in events) {
              [].forEach.call(events[y], function (evt) {
                if (
                  evt.listener
                    .toString()
                    .indexOf("function(cZ){cZ=cZ||R.event;") > -1
                ) {
      ...
      return {
        root: {
          "ec.europa.eu": [
            "ec.europa.eu\/index*",
            "ec.europa.eu\/about_*",
            "ec.europa.eu\/represent_*",
            "ec.europa.eu\/info",
            "ec.europa.eu\/priorities",
            "ec.europa.eu\/commission",
            "ec.europa.eu\/about",
            "ec.europa.eu\/atwork",
            "ec.europa.eu\/policies",
            "ec.europa.eu\/contracts_grants",
            "ec.europa.eu\/news",
            "ec.europa.eu\/legislation",
            "ec.europa.eu\/geninfo\/europa_analytics_*",
            "ec.europa.eu\/geninfo\/legal_notices_*",
            "ec.europa.eu\/green-papers",
            "ec.europa.eu\/white-papers",
            "ec.europa.eu\/cookies",
            "ec.europa.eu\/contact",
            "ec.europa.eu\/services",
            "ec.europa.eu\/your-rights",
            "ec.europa.eu\/visits",
            "ec.europa.eu\/sitemap",
          ],
    ...
    mobile: function (event) {
      ...
      if (
        el &&
        el.href &&
        !el.href.match(/^(javascript|mailto|#|sms)/i) &&
        !el.piwikTrackers &&
        el.href !== "" &&
        el.href !== location.href.split("#")[0] &&
        ($wt.isExternal(el.href) || $wt.isDocument(el)) &&
        !el.realClick &&
        !/ea_ignore|piwik_ignore/.test(el.className)
      ) {
        ...
        var validate =
          typeof window[callback] === "function" ? window[callback](el) : !0;
        ...
        $wt.analytics.timer = setTimeout(
          process,
          (params.mobile || {}).delay || 1000,
        );
      }
    },
    track: function () {
    ...
    isOptInModeEnable: function () {
      ...
      var domain =
        location.host.split(".").length > 3
          ? location.host.split(".").slice(-3).join(".")
          : location.host;
    ...
    optOut: function () {
      $wt.analytics.isOptInModeEnable()
        ? $wt.cookie.remove($wt.getSupercookieName("eu_optin"))
        : $wt.cookie.set({
    ...
    isTrackable: function () {
      if (
        $wt.analytics.isOptedOut() ||
        $wt.cookie.consent.status() === "refused" ||
        $wt.isDNTOn()
      ) {
        $wt.analytics.deletePiwikCookies();
        return !1;
    ...
    shouldNotTrack: function (params) {
      if ($wt.isDNTOn()) {
        ...
        var euCook = JSON.parse(
          decodeURIComponent($wt.cookie.get("eu_cookie_consent")),
        );
    ...
    run: function (obj, params) {
      ...
      $wt.getFile({
        type: "json",
        url:
          "https://eams.fpfis.tech.ec.europa.eu/config?siteID=" +
          params.siteID +
          "&instance=" +
          params.instance,
...
(function (win, doc, previous) {
  ...
  const trap = (cnt, key) => {
    let focusable = cnt.querySelectorAll(
      "a[href]:not([tabindex='-1']), input, select, textarea, button, [role='menuitem'], [role='button']",
    );
  ...
  var tab = function (elm, init, focus) {
    ...
    [].forEach.call(tabs.children || [], function (btn, index) {
      if (init) {
        btn.setAttribute(
          "tabindex",
          btn.getAttribute("aria-selected") === "true" ? 0 : -1,
        );
        btn.index = index;
        btn.onkeydown = function (e) {
          ...
          if (e.keyCode === 39) {
            ...
            current =
              tabs.children[btn.index - 1] ||
              tabs.children[tabs.children.length - 1];
        ...
        [].forEach.call(
          doc.querySelectorAll("." + cntrl + ", #" + cntrl),
          function (item) {
      ...
      if (!head.index) {
        ...
        head.onkeydown = function (e) {
          ...
          if (e.keyCode === 40) {
            ...
            current =
              heads.children[head.index - 1] ||
              heads.children[heads.children.length - 1];
          } else if (e.keyCode === 36) {
  ...
  var aria = function (container) {
    ...
    let isSelf = container.getAttribute
    ...
    [].forEach.call(dom, function (elm) {
      ...
      if (!ECL && !elm.get) {
        ...
        var controls = function (e) {
          ...
          reset(e);
          var brothers = doc.querySelectorAll(
            '[aria-controls="' + cntrl + '"]',
          );
          elm.FROM = brothers;
            ...
            if (child.tagName === "LINK" && cntrl === child.id) {
              ...
              var animated = (function () {
                if ((child.className || "").indexOf("animated [") !== -1) {
                  elm.animated = child.className
                    .replace(/(.*)animated\ \[(.*)\](.*)/gi, "$2")
                    .split(",");
  ...
  aria.run = (obj) => {
    let container = obj.params.include
      ? document.querySelector(obj.params.include)
      : document;
  ...
  aria.setAttributes = (elm, params = {}) => {
    Object.keys(params).forEach((prop) => {
      elm.setAttribute(
        "aria-" + $wt.cleanHTML(prop),
        $wt.cleanHTML(params[prop].toString()),
      );
    });
  };
  aria.trap = trap;
  ...
  $wt.aria.menu = (controller, callback = () => {}) => {
    let menuNode = document.querySelector(
      '[id="' + controller.getAttribute("aria-controls") + '"]',
    );
    if (!menuNode) {
      return;
    }
  ...
  $wt.aria.autocomplete = (
    ...
    let id = $wt.id();
    let targetID =
      field.getAttribute("aria-owns") || "wt-autocomplete--target-" + id;
    let spinner =
      '<span class="wt-icon--spinner wt-icon--rotate-animate wt-icon--fluid"></span>';
    let menuItems = [];
    let firstItem = !1;
    let lastItem = !1;
    let hover = !1;
    ...
    statement.setAttribute(
      "style",
      `border:0;clip:rect(0 0 0 0);height:1px;margin:-1px 0 0 -1px;overflow: hidden;padding:0;position:absolute;width:1px`,
    );
    ...
    const focusNext = (elm) => {
      let index = menuItems.indexOf(elm);
      let item = elm === lastItem ? menuItems[0] : menuItems[index + 1];
      focusOn(item);
    };
    const itemKeyDown = (event) => {
      let tgt = event.currentTarget;
      ...
      } else if (field.value.trim() === "" && !options.showOnFocus) {
        closePopup();
        currentSearch = "";
      } else {
        switch (key) {
    ...
    const searchAndResult = async () => {
      ...
      if (term === currentSearch && !options.showOnFocus) {
        return;
      }
      if (term.length > 2 || options.showOnFocus) {
        [...document.querySelectorAll(".wt-autocomplete--wrapper")].forEach(
          (elm) => elm.close(),
        );
        menuItems = [];
    ...
    ["change", "input", "keydown"].map((evt) =>
      field.addEventListener(evt, itemKeyDown, !1),
    );
    if (options.showOnFocus) {
      field.addEventListener("focus", searchAndResult, !1);
      field.addEventListener("click", searchAndResult, !1);
      window.addEventListener(
        "click",
        () => {
          [...document.querySelectorAll(".wt-autocomplete--wrapper")].forEach(
...
$wt.forms.extend({
  ...
  _run: function (target, options) {
    ...
    if (needCSS) {
      let path = $wt.getStaticPath({
        fallback: $wt.root + "/webtools.forms.css",
        name: "forms",
        params: {},
        type: "css",
      });
      $wt.include(path, process);
    } else {
      process();
    }
    return this;
  ...
  _range: function (elm, params) {
    ...
    var update = function () {
      ...
      if (text) {
        ...
        thumb.style.marginLeft = 0 - (thumb.offsetWidth + 10) / 2 + "px";
      }
      if (!elm.check) {
        elm.check = !0;
        check();
          ...
          if (
            ...
            if (options.legend && label) {
              ...
              if (typeof options.legend === "string") {
                legend.setAttribute(
                  "style",
                  $wt.template("background-color: {color}", {
            ...
            if (tag.type === "radio" || tag.type === "checkbox") {
              ...
              tag.type === "range" &&
              config.class.indexOf("wt-form") > -1
            ) {
              $wt.forms._range(tag, {
                data: item.data,
              });
            } else if (
              ["div", "p", "ul", "button", "pre"].indexOf(item.tag) > -1 &&
              item.data
            ) {
...
(() => {
  const _replaceGlobalWidgetPath = (str, root) => {
    ...
    let placeholders =
      /{widget_path(:(\d{6})\/([0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}))?}/gm;
    ...
    for (const match of matches) {
      ...
      if (root.includes("/wcloud/preview/")) {
        str = str.replace(
          `{widget_path:${match[2]}/${match[3]}}`,
          rootReview + "/" + match[3],
        );
      } else {
        str = str.replace(
          `{widget_path:${match[2]}/${match[3]}}`,
          rootReview + "/" + match[2] + "/" + match[3],
        );
      }
    }
    return str;
  ...
  const _process = (ref) => {
    ...
    script.reset = () => {
      script.isProcess = !1;
      script.removeAttribute("data-process");
    };
    $wt.before(script, ref.obj);
    $wt.remove(ref.obj.snippet);
    ref.obj.remove();
    $wt.collect();
  ...
  const run = (obj) => {
    ...
    $wt.getFile({
      ...
      url: obj.params.url,
      error: (error) =>
        console.error("WTINFO: File was not found, check your url.\n", error),
      success: (response) =>
        _process({
          params: obj.params,
          obj: obj,
...
$wt.on(window, "wtReady", () => {
  ...
  const iconsRefresh = (group) => {
    let icons = document.querySelectorAll(`:not(svg)[class^="${group.match}"]`);
    if (!icons.length || isProcess) {
      return;
    }
    isProcess = !0;
    const imgAttributes = (src, targ) => {
      [...src.attributes].forEach((attr) => {
        let name = $wt.cleanHTML(attr.name, {
          xss: !0,
        });
        let strg = $wt.cleanHTML(attr.value, {
          xss: !0,
        });
        if (name === "data-alt") {
          targ.setAttribute("alt", src.getAttribute("data-alt") || "");
        } else if (strg && !name.startsWith("on")) {
          targ.setAttribute(name, strg);
        }
      });
    };
    const replaceIcons = () => {
      [...icons].forEach((elm) => {
        let firstClass = elm.className.split(" ")[0];
        let isOld = !firstClass.includes("--") ? "-" : "";
        let classReference = firstClass.split(group.match);
        if (!classReference[1]) {
          return;
        }
        let iconName = classReference[1].split('"')[0].split(" ")[0];
        let fullName = group.match + isOld + iconName;
        let xhtml = window
          .getComputedStyle(document.body)
          .getPropertyValue("--" + fullName);
        if (group.name === "flags") {
          elm.process = !0;
          elm.className = elm.className.replace(firstClass, "");
          let img = document.createElement("img");
          img.src = $wt.root + "/images/flags/" + iconName + ".svg";
          $wt.after(img, elm);
          imgAttributes(elm, img);
          img.className = "wt-img--flags";
          if (elm.className) {
            img.className += " " + elm.className;
          }
        } else if (xhtml) {
          elm.process = !0;
          elm.className = elm.className.replace(firstClass, "");
          elm.innerHTML = $wt.filterXss(xhtml);
          let dom = elm.querySelector("svg");
          let labelsBy = [];
          let id = "_" + $wt.id();
          dom.setAttribute("aria-hidden", !0);
          dom.setAttribute("focusable", !1);
          dom.setAttribute("xmlns", 'xmlns="http://www.w3.org/2000/svg');
          if (elm.className) {
            dom.setAttribute("class", elm.className);
          }
          if (group.attributes) {
            let viewBox = dom.getAttribute("viewBox");
            let attrs = Object.keys(group.attributes);
            if (viewBox) {
              attrs = attrs.filter((attr) => attr !== "viewBox");
            }
            attrs.forEach((attr) => {
              dom.setAttribute(attr, group.attributes[attr]);
            });
          }
          [...elm.attributes].forEach((attr) => {
            ...
            if (name === "data-title" || name === "title") {
              let title = document.createElement("title");
              title.innerHTML = strg;
              title.id = "title" + id;
              $wt.insertBefore(dom, title);
              labelsBy.push(title.id);
              ...
              if (title) {
                ...
                $wt.insertBefore(dom, desc);
              }
              labelsBy.push(desc.id);
            } else if (strg && !name.startsWith("on")) {
              dom.setAttribute(name, strg);
            }
          });
          if (group.name === "icons") {
            dom.setAttribute("fill", "currentColor");
          }
          dom.classList.remove("wt-icon--placeholder");
          if (labelsBy.length) {
            labelsBy.push(dom.getAttribute("aria-labelledby") || "");
            dom.setAttribute("aria-labelledby", labelsBy.join(" ").trim());
    ...
    loadCSS($wt.root + `/css/webtools.icons.${group.file}.css`)
      .then(replaceIcons)
      .catch(() => {
        console.warn(
          `WTERROR: The family ${group.name} was not loaded properly.`,
        );
        isProcess = !1;
        family = family.filter((row) => row.file !== group.file);
        loop();
      });
  };
  const loop = () => {
    clearTimeout(timer);
    timer = setTimeout(() => {
      let checkIcons = document.querySelectorAll(
        `:not(svg)[class^="wt-icon-"]`,
      );
      if (checkIcons.length && !isProcess) {
        family.forEach(iconsRefresh);
      }
    }, 25);
  };
  if ("MutationObserver" in window) {
...
$wt.on(window, "wtBeforeCollect", () => {
  $wt.getStaticPath = (config) => {
    let path = $wt.root;
    let fallback = config.fallback || "";
    let params = config.params || {};
    let name = ["sbkm", "share"].includes(config.name) ? "sbkm" : config.name;
    let plugins = fallback.includes("&plugins=");
    let ext = config.type;
    if ($wt.env === "local") {
      return fallback;
    } else if (name === "charts" && plugins) {
      return fallback;
    } else if (name === "map" && params.version === "2.0" && plugins) {
      return fallback;
    }
    path += "/" + ext;
    path += "/webtools." + name;
...
$wt.on(window, "wtReady", () => {
  ...
  if (
    loc.protocol !== "https:" ||
    loc.href.indexOf("/wcloud/") > -1 ||
    loc.href.indexOf("/tests/cypress") > -1 ||
    ["localhost", "127.0.0.1"].indexOf(loc.hostname) >= 0 ||
    loc.hostname.indexOf("-ewcms.ci.fpfis.tech.ec.europa.eu") > -1 ||
    loc.hostname.indexOf(".c9.fpfis.tech.ec.europa.eu") > -1 ||
    loc.hostname.indexOf(".translate.goog") > -1 ||
    loc.hostname.indexOf(".acc.fpfis.tech.ec.europa.eu") > -1 ||
    loc.hostname.indexOf(".prd.fpfis.tech.ec.europa.eu") > -1 ||
    loc.hostname.indexOf(".tst.fpfis.tech.ec.europa.eu") > -1 ||
    loc.hostname.indexOf("comm-ewcms-uat-eu.fpfis.tech.ec.europa.eu") > -1 ||
    loc.hostname.indexOf("comm-ewcms-uat-ec.fpfis.tech.ec.europa.eu") > -1 ||
    loc.hostname.indexOf("comm-ewcms-uat-eu.acc.fpfis.tech.ec.europa.eu") >
      -1 ||
    loc.hostname.indexOf("comm-ewcms-uat-ec.acc.fpfis.tech.ec.europa.eu") >
      -1 ||
    loc.hostname.indexOf(".ngrok-free.app") > -1 ||
    loc.hostname.indexOf(".vfs.cloud9.eu-west-1.amazonaws.com") > -1 ||
    loc.hostname.indexOf(".googleusercontent.com") > -1 ||
    loc.hostname.indexOf("-cicd.ci.dhs.tech.ec.europa.eu") > -1 ||
    loc.hostname.indexOf(".acc.dhs.tech.ec.europa.eu") > -1 ||
    loc.hostname.indexOf(".prd.dhs.tech.ec.europa.eu") > -1 ||
    loc.hostname.indexOf("demo-ec.ewpp.ec.europa.eu") > -1 ||
    loc.hostname.indexOf("s3platform.jrc.ec.europa.eu") > -1 ||
    isIpAddress(loc.hostname)
  ) {
    return;
  }
  clearTimeout(window.wtInventoryTimer);
  window.wtInventoryTimer = setTimeout(() => {
    let inventory = Object.keys($wt.components)
      .filter((comp) => {
        return (
          [
            "piwik",
  ...
  function isIpAddress(input) {
    ...
    const ipv6Regex =
      /^(([0-9a-fA-F]{1,4}:){7}([0-9a-fA-F]{1,4}|:)|(([0-9a-fA-F]{1,4}:){1,7}:)|([0-9a-fA-F]{1,4}:){1,6}(:[0-9a-fA-F]{1,4}){1,6})$/;
...
$wt.on(window, "wtReady", () => {
  const removeLazyLoadingForImages = function () {
    let shouldRemoveLoading = !!$wt.url.params.get("wteagerimages");
  ...
  removeLazyLoadingForImages();
});
$wt.ready(function () {
  let wtenv = $wt.url.params.get("wtenv");
  if (wtenv && !window.wtenv) {
    window.wtenv = !0;
    document.querySelector('script[src*="/load.js"]').remove();
    let env = JSON.parse(
      '{"acc":"https://comm-euwtsml.acc.dhs.tech.ec.europa.eu"}',
    );
    Object.keys(env).forEach((key) => {
      env[key] += "/load.js";
    });
    if (env[wtenv]) {
      return $wt.include(env[wtenv]);
    } else {
      console.error("WT - Can't load environment wtenv:", wtenv);
      return;
    }
  }
  if ($wt.duplicate) {
  ...
  $wt.urlParams = $wt.getUrlParams();
  let SML = document.querySelector('script[src*="/load.js"]');
  let url = SML.src.split("/load.js")[0];
  let theme =
    $wt.url.params.get("theme") || $wt.url.params.get("theme", SML.src) || null;
  $wt.theme = $wt.setTheme(theme);
  $wt.spa = !!$wt.url.params.get("spa", SML.src) || !1;
  $wt.infogram = $wt.url.params.get("infogram", SML.src) || !1;
  $wt.root = url.startsWith("https://europa.eu/webtools")
    ? "https://webtools.europa.eu"
    : url;
  $wt.lang();
  ...
  $wt.defer(() => $wt.trigger(window, "wtReady"), 25);
});
